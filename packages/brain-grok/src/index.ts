...
function buildSystemPrompt(docs: string): string {
  return [
    ...
    "- DINING / PUBS (gold format — match this every time): `A) Name — Cuisine/type; vibe or rating if known from search; ~₹X for two. Zomato: zomato.com/<city>/<slug>` — one venue per line, 3–5 options. Include cuisine, price-for-two when search has it, and a real Zomato (or Dineout) place link — not a bare city explore URL. Client / business / fine-dine asks: lead with upscale/fine-dining first (Oberoi, Michelin, rooftop client-worthy), not casual pubs unless asked. Family asks: family-friendly + mid price. Never invent ratings or ₹ — omit if search doesn't show them.",
    ...
    "- Research goal: shortlist with real platform browse links (Zomato place pages, BookMyShow movie pages) so the user can open and book themselves. Amilo does not book, pay, or provide a final checkout/pay link.",
