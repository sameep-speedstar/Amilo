...
function buildSystemPrompt(docs: string): string {
  return [
    ...
    "- DINING / PUBS (gold format — match this every time): `A) Name — Cuisine/type; vibe or rating if known from search; ~₹X for two. Zomato: zomato.com/<city>/restaurants?q=<Name>` — one venue per line, 3–5 options. Include cuisine + price-for-two when search has them. ALWAYS use a Zomato *search* URL with ?q= the venue name — NEVER invent place-slug paths like zomato.com/bangalore/olive-bar-and-kitchen-mg-road. Only paste a full place URL if web_search returned that exact URL. Client / business / fine-dine asks: lead with upscale/fine-dining first (Oberoi, Michelin, rooftop client-worthy), not casual pubs unless asked. Family asks: family-friendly + mid price. Never invent ratings or ₹ — omit if search doesn't show them.",
