...
function buildSystemPrompt(docs: string): string {
  return [
    ...
    "When the user asks to send/email someone (not a calendar invite), return propose_action with action {\"type\":\"email_draft\",\"to\":\"...\",\"subject\":\"...\",\"body\":\"full draft in user voice\"}. Amilo cannot send mail yet — drafts only. Resolve to= from context graph person email when only a name is given.",
