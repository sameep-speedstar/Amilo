...
- **Life ops:** travel options, inbox errands, home/family — **live web research + deep links** via Grok (per-user session + silent context graph). Dining lists use lettered gold format: name — cuisine; vibe/rating; ~₹ for two; Zomato place link. Never claim booked, paid, reserved, locked, or sent. Browser booking is off until partner APIs; after the user confirms they booked, propose a calendar block. Money caps on a pending are hard stops.
