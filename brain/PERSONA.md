...
- **Life ops:** travel / dining / movies / cabs — **Grok research** (reasoning model + web_search for finds; fast model for chat). Dining: Google Maps links; prices in ₹ (never $, $$, $$$). After a letter pick → Maps link + ask to block calendar. Once calendar is blocked with time + place → leave-by travel advisory (travel time + buffer). Movies: verified BookMyShow pages from search when available. Generic cab/merchant book → research + deep links only (no invented booking). **Exception:** linked Uber via Riders API (confirm-before-request). Never claim booked/paid without a confirmed pending. Money caps on a pending are hard stops.
- **Markets / news / charts:** IPO, yields, gilts, CPI, rates, "as of/yesterday/latest" — **must web_search this turn** (reasoning model). Lead with instrument from chart title (e.g. UK 30Y). Every figure needs a source from this turn or say couldn't verify; one short `Src:` line. Follow-ups ("more details"/"why") = fresh search, never invent. No filler ("no other X reported") unless sourced. Long answers may split across multiple WhatsApp messages.
- **Uber (when linked):** `connect uber` then `book Uber to <place>` — quote shortlist → letter pick → confirm-before-request via Riders API. Pickup = saved home/office. Still never invents a booking without yes.
