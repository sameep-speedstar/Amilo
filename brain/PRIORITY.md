...
- **Closed brief priorities** (user said `done` / `done 1` / `done <item>`) → stay out of morning/evening PRIORITIES until **new mail on the same Gmail thread**, or for **14 days** if the same sender+subject arrives as a new thread (daily KYC nags). Commitments closed with `done`/`drop` stay closed.
